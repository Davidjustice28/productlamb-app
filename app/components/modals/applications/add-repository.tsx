...
export function PLNewRepositoryComponent({applicationId, onRepositoriesChange}: {applicationId: number, onRepositoriesChange?: (repositories: RepositoryCreationBaseInfo[]) => void}) {
  const [platform, setPlatform] = useState<"Github" | "Gitlab">("Github")
  const [repositories, setRepositories] = useState<(GithubRepositoryInfo| GitlabRepositoryInfo)[]>([])
      ...
      <div className="flex flex-col gap-2 w-full border-t-2 border-neutral-400 mt-5 py-5">
        {
          ...
          repositories.map((repo, index) => (
            <div key={index} className="flex flex-row gap-3 text-black dark:text-neutral-300">
              <p>
